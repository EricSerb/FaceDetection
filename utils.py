...
def clean_compile(res_dir):
    ...
    try:
        ...
        # call(["make", "clean"])
        # call(["make"])
        p = Popen(["make", "clean"], stdout=PIPE, stderr=PIPE)
        stdout, stderr = p.communicate()
        logger.info("Output of make clean stdout:")
        logger.info("\n{}".format(stdout))
        logger.warning("Output of make clean stderr:")
        logger.warning("\n{}".format(stderr))
        p.wait()
        p = Popen(["make"], stdout=PIPE, stderr=PIPE)
        stdout, stderr = p.communicate()
        logger.info("Output of make:")
        logger.info("\n{}".format(stdout))
        logger.warning("Output of make errors:")
        logger.warning("\n{}".format(stderr))
        p.wait()
    except OSError as e:
